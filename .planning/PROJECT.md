...
> **Context change (2026-09-10): this is now a commercial product.** The owner intends to sell access. That raises three things from optional to mandatory: measured accuracy (you cannot sell a quality claim you have never tested), hard spend caps (unit cost is now margin), and access control (every unauthorized question is a direct loss). The eval harness, previously deferred to v2, is pulled into v1 scope for this reason. Access is sold via a manually managed allowlist — the owner adds each buyer with `/adduser` after payment.
>
> Two commercial risks recorded, not resolved: "SAT" is a College Board trademark and their practice questions are their copyright, so a paid product built on both carries IP exposure worth legal review before launch. And the product's honest output is sometimes "unresolved" — marketing must promise *model consensus*, never *correct answers*, or contested splits become refund requests.
...
| Allowlist-only, no open access | Originally cost control for a study group; now the paywall itself. Buyers are added manually with `/adduser` after payment. | — Pending |
| Pull the eval harness into v1 | Selling a bot whose accuracy has never been measured is the central commercial risk. Ground truth comes from a small labeled set (5-10 official questions to start) plus `ground_truth` accumulated by the correction flow. | — Pending |
| Manual allowlist over access codes or Telegram Stars | Owner's choice. Simplest to build and ship; the tradeoff is the owner is the bottleneck on every sale, which starts hurting past roughly 20 customers. | — Pending |
| Reply states the answer plainly, no spoiler | Owner request during live testing. Reverses CON-06. The tally moved to a footer and every model's vote is now listed. Agreement percentage is derived from vote share, never from model self-reported confidence, which research showed is miscalibrated. | — Pending |
| 45-second round budget, 42s per model | Measured: Opus 5 alone ranged 19.4s / 22.1s / 32.6s across three live runs. The original 26s round budget would have dropped it intermittently. | ✓ Good |
| `deepseek/deepseek-v4.1-flash` → `mistralai/mistral-large-2512` | DeepSeek's vision endpoint is persistently 429 rate-limited on the shared pool (text-only succeeds), which silently reduced the round to 3 labs while boot validation still reported ok. Mistral Large tested at 6.9s, parses cleanly, $0.002. | ✓ Good |
| `reasoning.effort` is per-model, not global | `gpt-6-astra` and `gemini-3.7-flash` reject `none` outright ("Reasoning is mandatory for this endpoint"). Both use `minimal`; Anthropic omits the key entirely. Only discoverable against live models. | ✓ Good |
