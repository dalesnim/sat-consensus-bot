...
- [x] **DATA-01**: Bot computes a perceptual hash of every submitted image at `hash_size=16`
- [x] **DATA-02**: An exact perceptual hash match returns the stored result with no inference; no fuzzy Hamming-distance matching in v1
...
- [x] **DATA-05**: Replies served from cache are flagged as such in persistence
...
- [x] **ACC-04**: A global daily spend cap is read from the environment and enforced atomically, so concurrent requests cannot race past it
- [x] **ACC-05**: Above the spend cap the bot drops to a reduced cheaper model set and says so in the reply
