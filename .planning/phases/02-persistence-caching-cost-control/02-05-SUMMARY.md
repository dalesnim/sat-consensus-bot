...
> 2. **/adduser works.** As the owner, send `/adduser <second account's id>`. Expected: a confirmation naming the id. Resend the photo from the second account. Expected: a normal seven-model consensus reply.
> 3. **/adduser is owner-only.** From the second account, send `/adduser 999999999`. Expected: the same generic refusal, no confirmation.
> 4. **Persistence.** Run `sqlite3 data/bot.db "SELECT id, user_id, consensus_state, min_transcription_overlap FROM questions;"` and `sqlite3 data/bot.db "SELECT model_id, answer, latency_ms, cost_usd, error FROM attempts;"`. Expected: one question row attributed to the second account, **seven** attempt rows with non-null `latency_ms` and `cost_usd` (the roster is seven models, not the six this plan was written against; an eighth row appears only if the round split and `claude-fable-5.1` was called as tiebreak).
> 5. **Cache is free.** Send the byte-identical photo again from either account. Expected: a reply within about a second, identical text to the first. Then `sqlite3 data/bot.db "SELECT id, served_from_cache, source_question_id FROM questions;"` — expected: a second row with `served_from_cache = 1` pointing at the first, and still the same seven rows in `attempts` — a cache hit adds no attempt rows.
...
> 7. **Spend cap degrades.** Temporarily set `DAILY_SPEND_CAP_USD` to just above the day's spend so far (read it from `/cost`), restart, and send a new distinct question. Expected: a normal answer ending with `Today's budget cap was reached, so a smaller, cheaper model set answered this one.` (the line deliberately names no count — see commit `eb1abdb`). The per-model list shows **five** models — `grok-4.6`, `gemini-3.1-pro-preview`, `claude-sonnet-5`, `qwen3.8-max-0902`, `gemini-3.7-flash` — with `claude-opus-5` and `gpt-6-astra`, the two most expensive, absent. Four distinct labs remain represented.
...
> 9. **/cost reads correctly.** Restore `DAILY_SPEND_CAP_USD` to its real value and run `/cost` as the owner. Expected: today's spend, the cap, last 7 days, question count with the cache count and hit rate, cost per question to four decimal places, and a line saying figures reset at 00:00 UTC. Cross-check `cost_per_question_today` against the ~$0.247 measured baseline for the seven-model roster (the $0.11 figure in the plan predates the accuracy bakeoff) — a wildly different number means the cost accounting is wrong, not that the models got cheaper.
